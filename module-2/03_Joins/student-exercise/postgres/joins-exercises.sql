...
SELECT store_id, address, staff.first_name, staff.last_name
FROM staff
...
SELECT customer.first_name, customer.last_name, COUNT(rental_id) AS rentals
FROM customer 
...
SELECT store.store_id, address.address, COUNT(rental.rental_id), SUM(payment.amount), ROUND(SUM(payment.amount)/COUNT(payment.rental_id),2) AS average_sale
FROM payment
...
SELECT (actor.first_name || ' ' || actor.last_name) as actor_name, COUNT(rental.rental_id)
FROM actor
...
SELECT (actor.first_name || ' ' || actor.last_name) as actor_name, COUNT(rental_id)
FROM actor
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film_actor.film_id = film.film_id
JOIN film_category ON film.film_id = film_category.film_id
