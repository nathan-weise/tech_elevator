...
SELECT ROUND(((gnp * 1000000) / population), 2) AS gnp_per, name
FROM country
WHERE population != 0
...
SELECT ROUND(CAST(AVG(population)AS numeric),4) 
FROM city
...
SELECT ROUND(CAST((population / surfacearea) AS numeric),4) AS pop_density
FROM country
...
SELECT ROUND(CAST((population / surfacearea) AS numeric),4) AS pop_density, lifeexpectancy
FROM country
...
SELECT gnp - gnpold AS gnp_difference, ABS(gnp - gnpold) AS gnp_absolute
FROM country
ORDER BY gnp_absolute;

-- 17. The average population of cities in each country (hint: use city.countrycode)
-- ordered from highest to lowest.
-- (highest avg population: 4017733.0000, "SGP")

SELECT ROUND(CAST(AVG(population) AS numeric),4) AS pop_avg
FROM city
...
SELECT ROUND(CAST(AVG(lifeexpectancy)AS numeric),4), MIN(lifeexpectancy), MAX(lifeexpectancy), continent
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
