...
Once the files in the domain have been indexed, you can search for words. Add a call to `SearchEngine`'s `search()` method below the `Step Six` comment line in `Application.java`. Start with single words—for example, `search("squirrel")` or `search("Larry")`.
