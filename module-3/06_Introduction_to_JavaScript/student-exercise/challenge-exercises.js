...
function iqTest(string) {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        if (string.length === 0) {
            return 0;
        } else if (oddNums.length === evenNums.length) {
            return 0;
        } else if (oddNums.length === 1) {
            return oddPlace;
        } else if (evenNums.length === 1) {
            return evenPlace;
        } else {
            return 0;
        }
...
function titleCase(string, exceptions) {
    ...
    let array2 = exceptions.split(' ');
    let result = [];
    for(let i = 0; i < array1.length; i++) {
        if(i === 0) {
            result.push((array1[i].charAt(0)).toUpperCase() + (array1[i].slice(1)).toLowerCase());
        } 
