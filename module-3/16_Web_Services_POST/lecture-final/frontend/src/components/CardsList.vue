      ...
      if (confirm('Are you sure you want to delete this board? This cannot be undone!')) {
        boardsService.deleteBoard(this.boardId).then(response => {
          if (response.status === 200) {
            alert('Board has been destroyed. Good job.');
            this.$store.commit('DELETE_BOARD', this.boardId);
            this.$router.push({name: 'Home'});
          }
        }).catch(error => {
          if (error.response) {
            this.errorMsg = 'Error deleting board. Reponse was ' + error.response.data.message;
          } else if (error.request) {
            this.errorMsg = 'Error deleting board. Server did not respond.';
          } else {
            this.errorMsg = 'Error deleting board. Failed to send request.';
          }
