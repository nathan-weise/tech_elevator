<template>
  <div id="sideNav">
    ...
    <div class="boards">
      ...
      <form v-on:submit.prevent v-if="showAddBoard">
        Board Title:
    ...
    saveNewBoard() {
      boardsService.addBoard(this.newBoard).then(response => {
        if (response.status === 201) {
          ...
          this.retrieveBoards();
          alert('New Board was created successfully.');
        }

      }).catch(error => {
        if (error.response) {
          this.errorMsg = 'Failed to create board. Reponse was: ' + error.response.data.message;
        } else if (error.request) {
          this.errorMsg = 'Couldn\'t create board. Server did not respond.';
        } else {
          this.errorMsg = 'Something went really wrong. Couldn\'t send request.';
        }
